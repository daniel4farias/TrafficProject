...
f.each_line do |line|
  if line =~ /<edge id="(.*)"\s*from="(.*)"\s*to="(.*)"\s*type="(.*)"\/>/
    edges[$1] = [$2, $3]
  end
end
f.close
...
while index < lines.length
  if lines[index] =~ /<fromEdge\s*id="(.*)">/
    src = $1
    turns[src] = {}
    count = 1
    while lines[index+count] =~ /<toEdge\s*id="(.*)"\s*probability="(.*)"\/>/
      newprobability = $2
      newedge = $1
  ...
  if lines[index] =~ /<source\s*nodes="(.*)"\s*rate="(.*)"\/>/
    sources[$1] = $2#.push($1)
  end
  if lines[index] =~ /<sink\s*nodes="(.*)"\/>/
    sinks.push($1)
  end
  index += 1
end
  ...
  sources.keys.each { |source|
    ...
    if routes[source][i] and rule(rate(sources[source].to_f, start_time), rand)
      ...
      if rand.rand < autonomous[0]/100 # if autonomous
        f.write("  <vehicle depart=\"#{start_time}\" id=\"veh#{vehicle_id}\" route=\"#{routes[source][i][2]}\" type=\"CarA\"/>\n");  
      else # if manual vehicle
        f.write("  <vehicle depart=\"#{start_time}\" id=\"veh#{vehicle_id}\" route=\"#{routes[source][i][2]}\" type=\"CarM\"/>\n");
      end
    end
  }
end
